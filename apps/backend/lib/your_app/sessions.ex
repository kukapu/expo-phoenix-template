defmodule YourApp.Sessions do
  ...
  defp build_session(user, session_id, refresh_token) do
    ...
    %{
      access_token:
        Phoenix.Token.sign(YourAppWeb.Endpoint, YourApp.Auth.access_token_salt(), %{
          sub: user.id,
  ...
  defp insert_refresh_token(repo, session_family, parent_token, current_time) do
    ...
    expires_at = DateTime.add(current_time, YourApp.Auth.refresh_token_ttl_days(), :day)
  ...
  defp hash_refresh_token(plain_refresh_token) do
    :sha256
    |> :crypto.hash(YourApp.Auth.refresh_token_salt() <> plain_refresh_token)
