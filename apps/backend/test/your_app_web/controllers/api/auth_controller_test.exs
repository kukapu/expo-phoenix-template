defmodule YourAppWeb.Controllers.Api.AuthControllerTest do
    ...
    test "creates the user and returns a backend session bundle on first login", %{
      ...
      provider_token =
        ProviderTokenFactory.google_token!(provider_config.google_key, %{
          ...
          "email" => "google-controller-first@google.your_app.test"
        })
      ...
      assert %{
               ...
               "user" => %{"email" => "google-controller-first@google.your_app.test"}
    ...
    test "returns the existing user on returning login without duplicating the account", %{
      ...
      provider_token =
        ProviderTokenFactory.google_token!(provider_config.google_key, %{
          ...
          "email" => "google-controller-returning@google.your_app.test"
        })
    ...
    test "accepts Apple callbacks when the nonce is present", %{
      ...
      provider_token =
        ProviderTokenFactory.apple_token!(provider_config.apple_key, "nonce-123", %{
          ...
          "email" => "apple-controller-user@apple.your_app.test"
        })
      ...
      assert %{
               "accessToken" => access_token,
               "refreshToken" => refresh_token,
               "user" => %{"email" => "apple-controller-user@apple.your_app.test"}
             } = json_response(conn, 201)
